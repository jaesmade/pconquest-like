...
`src/game/engine.ts` appends an attack visual event when a move is used. The event contains its move ID, attacker tile, selected target tile, affected tiles, and units hit. `src/battle/Board.tsx` reads those events in order. A projectile launches after a short windup and moves between tile centers; area moves appear on every affected tile, while weather moves use a sparse board-wide cue. The board keeps player controls inactive until queued attack effects finish. The combat result is calculated immediately by the rules, independently of playback.
...
The current battle scene faces the attacker along the first differing grid axis, draws the tile and elevation before the unit, and puts attack effects above sprites. Attack visual events are played in order, including those produced by consecutive enemy turns. The rules and round order determine results; animations display those results and temporarily lock player commands until playback ends.
