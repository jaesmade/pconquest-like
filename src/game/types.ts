...
export type Battle = { map: BattleMap; tileChanges: Record<string, TileChange>; objective: Encounter['objective']; units: Unit[]; weather: Weather; weatherUntil: number; time: number; round: number; turnOrder: string[]; turnIndex: number; current: string; rngState: number; log: string[]; visualEvents: AttackVisualEvent[]; result?: 'win' | 'loss'; captureHeld: boolean; encounterId: string };
