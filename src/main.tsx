...
function App({ initialRun }: { initialRun: Run }) {
  ...
  return <div className={animating ? "app-shell animating" : "app-shell"}>
    <header className="topbar"><div><span className="eyebrow">TACTICAL ROGUELIKE · EARLY BUILD</span><h1>Pokémon Tactics</h1></div><div className="top-status">{saveFailed && <span role="alert">Progress could not be saved in this browser. </span>}{run.phase !== 'starter' && <>Encounter {Math.min(run.encounter + 1, ENCOUNTERS.length)} / {ENCOUNTERS.length} · Party {run.party.length} / 6 <button className="reset-run" onClick={() => { setRun(freshRun(run.unlocks)); resetSelection(); setAnimating(false); }}>New run</button></>}</div></header>
    ...
    {run.phase === 'prepare' && <PrepareScreen run={run} onToggle={toggleDeploy} onEquipMove={equipMove} onEquipItem={equipItem} onDeploymentChange={deployment => patch(next => { next.deployment = deployment; })} onStart={start} />}
