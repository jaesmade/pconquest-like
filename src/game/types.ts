...
export type MoveEffect =
  ...
  | { kind: 'stage'; on: 'cast'; stat: 'attack' | 'defense'; delta: number; recipients: 'self' | 'allies' | 'all' }
...
export type Unit = { id: string; partyId?: string; side: 'player' | 'enemy'; species: string; name: string; types: string[]; mobility: Mobility; ability: AbilityId; stats: [number, number, number, number, number, number, number]; moves: string[]; hp: number; maxHp: number; x: number; y: number; facing: number; ap: number; maxAp: number; attackedThisTurn: boolean; status: Record<string, number>; stages: { attack: number; defense: number }; item: ItemId; itemAttackMultiplier: number; mega: boolean; visual?: string; visualNonce?: number; visualPath?: GridPoint[] };
