...
export default function PrepareScreen({ run, onToggle, onEquipMove, onEquipItem, onDeploymentChange, onStart }: Props) {
  ...
  return <main className="narrow">
    ...
    <section className="deployment-panel" aria-label="Deployment zones">
      <div className="deployment-head"><div><span className="eyebrow">DEPLOYMENT</span><h3>Choose starting tiles</h3><p>Select a Pokémon, then choose a green tile in the bottom ally rows. Enemies start in the top rows, with neutral ground between. Occupied tiles swap positions when both Pokémon can use them. This planning grid is shown from above.</p></div><div className="deployment-legend"><span>● Ally</span><span>● Neutral</span><span>● Enemy</span></div></div>
